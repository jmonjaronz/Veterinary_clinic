...
export class AppointmentsService {
    ...
    async findAll(filterDto?: AppointmentFilterDto) {
        ...
        if (filters.pet_id) {
            queryBuilder.andWhere('appointment.pet_id = :pet_id', { pet_id: filters.pet_id });
        }
        
        if (filters.veterinarian_id) {
            queryBuilder.andWhere('appointment.veterinarian_id = :vet_id', { vet_id: filters.veterinarian_id });
        }
        ...
        if (filters.appointment_type) {
            queryBuilder.andWhere('appointment.appointment_type = :type', { type: filters.appointment_type });
        }
        ...
        if (filters.date_start && filters.date_end) {
            ...
            queryBuilder.andWhere('appointment.date >= :start', { start: filters.date_start });
        } else if (filters.date_end) {
            queryBuilder.andWhere('appointment.date <= :end', { end: filters.date_end });
        }
    ...
    async findByPet(petId: number, filterDto?: AppointmentFilterDto): Promise<any> {
        // Verificar si la mascota existe
    ...
    async findByVeterinarian(veterinarianId: number, filterDto?: AppointmentFilterDto): Promise<any> {
    ...
    async findByDateRange(startDate: Date, endDate: Date, filterDto?: AppointmentFilterDto): Promise<any> {
        // Crear una copia del filtro o uno nuevo si no hay
        const filters = filterDto ? { ...filterDto } : new AppointmentFilterDto();
