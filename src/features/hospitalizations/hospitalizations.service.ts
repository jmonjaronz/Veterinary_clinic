...
export class HospitalizationsService {
    ...
    async findAll(filterDto?: HospitalizationFilterDto) {
        ...
        if (filters.pet_id) {
            queryBuilder.andWhere('hosp.pet_id = :pet_id', { pet_id: filters.pet_id });
        }
        
        if (filters.veterinarian_id) {
            queryBuilder.andWhere('hosp.veterinarian_id = :vet_id', { vet_id: filters.veterinarian_id });
        }
        
        // Filtro por razón de hospitalización
        if (filters.reason_contains) {
            queryBuilder.andWhere('hosp.reason LIKE :reason', { reason: `%${filters.reason_contains}%` });
        }
        ...
        if (filters.admission_date_start && filters.admission_date_end) {
            queryBuilder.andWhere('hosp.admission_date BETWEEN :adm_start AND :adm_end', {
                adm_start: filters.admission_date_start,
                adm_end: filters.admission_date_end
            });
        } else if (filters.admission_date_start) {
            queryBuilder.andWhere('hosp.admission_date >= :adm_start', { adm_start: filters.admission_date_start });
        } else if (filters.admission_date_end) {
            queryBuilder.andWhere('hosp.admission_date <= :adm_end', { adm_end: filters.admission_date_end });
        }
        
        // Filtros de fechas de alta
        if (filters.discharge_date_start && filters.discharge_date_end) {
            queryBuilder.andWhere('hosp.discharge_date BETWEEN :dis_start AND :dis_end', {
                dis_start: filters.discharge_date_start,
                dis_end: filters.discharge_date_end
            });
        } else if (filters.discharge_date_start) {
            queryBuilder.andWhere('hosp.discharge_date >= :dis_start', { dis_start: filters.discharge_date_start });
        } else if (filters.discharge_date_end) {
            queryBuilder.andWhere('hosp.discharge_date <= :dis_end', { dis_end: filters.discharge_date_end });
        }
    ...
    async findByPet(petId: number, filterDto?: HospitalizationFilterDto): Promise<any> {
        // Verificar si la mascota existe
    ...
    async findByVeterinarian(veterinarianId: number, filterDto?: HospitalizationFilterDto): Promise<any> {
